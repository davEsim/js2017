...
class Items{
    ...
    public function getPath($id){
		...
		$lang = $_ENV["lang"];
		$path=$_ENV["serverPath"].$this->pagePath[$lang]."/".$id."-".string2domainName($r[$routing[$this->pagePath[$lang]]["metaTitle"]]);
		return $path;
    }
    ...
    public function listing($where="",$orderBy="id", $direction="DESC", $from=0, $count=6){
		...
		//echo "<tr><td>SELECT * FROM $this->table $where ORDER BY $orderBy $direction $limit</td></tr>";
    ...
    public function getRelCol($id, $relTable){
        $params=array(":id" => $id);
